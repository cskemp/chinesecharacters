...
This notebook contains analyses reported in Han, Kelly, Winters & Kemp, Simplification is not dominant in the evolution of Chinese characters. Before running this notebook please replace the `data` folder in the GitHub repository with the complete data folder downloaded from Zenodo.
...
# Path to the main complexity file 
...
d_cld <-  read_csv(dpath_cld) %>%
  ...
  select(Character, C1Type, C1Frequency, AdjustedC1Frequency, Strokes) %>%
  rename(character= Character,rawfrequency=C1Frequency, frequency=AdjustedC1Frequency, stroke_count=Strokes) %>%
...
d_c_meds <- d_c %>% 
  ...
  distinct(across(c("character", "period")), .keep_all = TRUE) 
...
This section generates Figure S15. See `descriptive_complexity.Rmd` for code that generates Figure S16.
...
ggsave(here("figures", "figs15_alt_complexity_measures.pdf"), plot = altcomplex,  width=7.08661,height = 2.8)
```
...
print(tableout, file = here("analysis", "output", paste0("coeftable_periodregression.tex")))
...
for (freqsuffix in c("", "raw")) {
  ...
  ggsave(here("analysis","output", paste0("bayesreg_freq", freqsuffix, ".pdf")), plot = postplot,  width=5,height = 5)

  tableout<- xtable(fixef(fit3freq_thissuffix), caption = "Coefficients for frequency regression" )
  print(tableout, file = here("analysis", "output", paste0("coeftable_frequencyregression", freqsuffix, ".tex")))
  ...
  postplot <- mcmc_areas(coefsamples)
  ggsave(here("analysis","output", paste0("bayesreg_freq_tradonly", freqsuffix, ".pdf")), plot = postplot,  width=5,height = 2.5)

  tableout<- xtable(fixef(fit3freq_tradonly_thissuffix), caption = "Coefficients for frequency regression (oracle vs traditional)" )
  print(tableout, file = here("analysis", "output", paste0("coeftable_frequencyregression_tradonly", freqsuffix, ".tex")))
}
```
...
print(tableout, file = here("analysis", "output", "coeftable_ctype.tex"))
...
print(tableout, file = here("analysis", "output", "coeftable_numcompregression.tex"))
...
ggsave(here("analysis","output", "bayesreg_period_distinctiveness.pdf"), plot = postplot,  width=4,height = 2.8)

tableout<- xtable(fixef(fit_d), caption = "Coefficients for period regression (distinctiveness)" )
print(tableout, file = here("analysis", "output", paste0("coeftable_periodregression_distinctiveness.tex")))
```
...
for (freqsuffix in c("")) {
  ...
  ggsave(here("analysis","output", paste0("bayesreg_freq_distinct", freqsuffix, ".pdf")), plot = postplot,  width=5,height = 5)

  tableout<- xtable(fixef(fit3freq_thissuffix), caption = "Coefficients for frequency regression" )
  print(tableout, file = here("analysis", "output", paste0("coeftable_frequencyregression_distinct", freqsuffix, ".tex")))
}
```
