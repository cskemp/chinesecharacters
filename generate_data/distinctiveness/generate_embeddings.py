...
def evaluate(net, imagepath):
    ...
    for img_name in allimage:
        ...
        character, period, image_id = img_name.split(".")[0].split("_")
        rows.append((character, period, image_id, list(net.blobs['cls3_fc1'].data[0])))

        count += 1
        print(count, total)
    
    df = pd.DataFrame(rows, columns=["character", "period", "image_id", "fc1_embedding"])
